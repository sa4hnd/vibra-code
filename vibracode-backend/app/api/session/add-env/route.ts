  ...
  try {
    ...
    if (!sessionId || !key || value === undefined) {
      return NextResponse.json(
        { error: 'Missing sessionId, key, or value' },
        { status: 400 }
      );
    }

    // Add the environment variable to the session in DB
    await fetchMutation(api.sessions.addEnv, {
      sessionId,
      key,
      value,
    });
...
EOF
      ...
      console.log(`Added env ${key} and synced to sandbox`);
      return NextResponse.json({ success: true, synced: true });
    } catch (syncError) {
      console.warn('Failed to sync env to sandbox:', syncError);
      return NextResponse.json({ success: true, synced: false });
    }
  } catch (error) {
