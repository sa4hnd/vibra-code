...
import { ArrowRightIcon, Smartphone, Apple } from "lucide-react";
import { Component as TrustedBy } from "./trusted-by";
...
export type HeroWaveProps = {
  ...
  placeholder?: string;
  buttonText?: string;
  onPromptSubmit?: (message: string, repository?: any, imageData?: any, templateId?: string) => void;
...
export function HeroWave({
  ...
  style,
  extendLeftPx = 320,
  title = "Build Mobile Apps with AI.",
  ...
  // Update prompt when initialPrompt changes (e.g., after sign-in)
  useEffect(() => {
  ...
  // Prompt suggestions for quick selection
  const promptSuggestions = [
    "Create a simple calculator mobile app",
    "Build a basic note-taking mobile app",
    "Make a simple timer and stopwatch mobile app",
    "Create a basic flashlight mobile app",
    "Build a simple color picker mobile app",
    "Make a basic unit converter mobile app"
  ];
  ...
  const suggestionsRef = useRef<string[]>([
    ...
    " basic note-taking mobile app",
    " simple timer mobile app",
    " basic flashlight mobile app",
    " simple color picker mobile app",
    " basic unit converter mobile app",
        ...
        >
          ...
          <div className="mb-8 mt-0">
            <Banner
              show={showBanner}
              variant="gradient"
              title="Use our mobile app to build mobile apps"
              showShade={true}
              closable={false}
              icon={
              ...
              action={
                <a
                  href="https://apps.apple.com/us/app/vibra-code-ai-app-builder/id6752743077"
                  target="_blank"
                  rel="noopener noreferrer"
                  className="inline-flex items-center gap-1 rounded-full bg-white/10 px-3 py-1.5 text-sm font-medium transition-all duration-200 hover:bg-white/20 border border-white/10"
                >
                  Download
                  <ArrowRightIcon className="h-3 w-3" />
                </a>
              }
              className="max-w-md mx-auto bg-black/20 border-white/10 backdrop-blur-sm"
            />
          </div>
          ...
          <h1 className="text-white text-3xl sm:text-5xl font-semibold tracking-tight drop-shadow-[0_1px_8px_rgba(31,61,188,0.25)]">
            {title.includes('masterpiece') ? (
              <>
                {title.split('masterpiece')[0]}
                <span className="font-black inline-block transform rotate-2 text-4xl sm:text-6xl" style={{ fontFamily: 'Lobster, cursive' }}>
                  masterpiece
                </span>
                {title.split('masterpiece')[1]}
              </>
            ) : (
              title
            )}
          ...
          <form
            ...
            onSubmit={async (e) => {
              ...
              try {
                await onPromptSubmit?.(prompt, undefined, undefined, "expo");
              } finally {
          ...
          <div className="mt-4 sm:mt-6">
            <p className="text-gray-300/70 text-sm mb-3">Try these prompts:</p>
            <div className="flex flex-wrap gap-2 justify-center">
              {promptSuggestions.map((suggestion, index) => (
                <button
                  ...
                  className="px-3 py-2 text-sm bg-white/5 hover:bg-white/10 border border-white/10 rounded-lg text-gray-300 hover:text-white transition-all duration-200 backdrop-blur-sm disabled:opacity-50 disabled:cursor-not-allowed"
                >
                  {suggestion}
                </button>
              ))}
