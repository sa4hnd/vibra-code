...
export function HomeScreen(props: NavigationProps) {
  ...
  const { recentHistory, allHistory } = useSelector(
  ...
  // Use Clerk auth state instead of legacy Redux session
  const { isAuthenticated } = useVibraAuth();
  ...
  return (
    <View style={{ flex: 1 }}>
      <View style={{ backgroundColor: 'transparent', height: insets.top }} />
      <HomeScreenView
        theme={themeType}
        {...props}
        isFocused={isFocused}
