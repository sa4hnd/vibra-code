...
export async function setPreviewZoomAppNameAsync(appName: string): Promise<void> {
  if (NativeKernel.setPreviewZoomAppName) {
    await NativeKernel.setPreviewZoomAppName(appName);
  }
}
