...
- (void)createRootAppAndMakeVisible
{
  ...
  if ([EXBuildConstants sharedInstance].isDevKernel) {
    ...
    // Use EAS Update instead of embedded bundles
    NSURL *easUpdateUrl = [NSURL URLWithString:@"https://u.expo.dev/YOUR_EAS_PROJECT_ID"]; // Replace with your EAS project ID
    homeAppLoader = [[EXAppLoaderExpoUpdates alloc] initWithManifestUrl:easUpdateUrl];
  }
...
- (void)reloadVisibleApp
{
  ...
  // Note: Orange menu will be automatically shown by EXKernel when the new app becomes visible
}
...
{
  // Dev menu disabled - commented out dev menu opening
  // show nux if needed
