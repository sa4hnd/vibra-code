...
- (BOOL)open
{
  // Dev menu disabled - prevent opening
  return NO;
...
{
  // Dev menu disabled - prevent showing window
  if (visible) {
    return; // Don't show dev menu window
  }
  ...
  dispatch_async(dispatch_get_main_queue(), ^{
    ...
    // Update orange menu window level when dev menu visibility changes
    [[EXOrangeMenuManager sharedInstance] updateWindowLevel];
  });
}
