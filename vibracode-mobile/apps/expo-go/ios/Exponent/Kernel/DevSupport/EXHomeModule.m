...
/**
 * Sets the app name to display in the preview zoom top bar (from Convex database).
 */
RCT_EXPORT_METHOD(setPreviewZoomAppName:(NSString *)appName
                  resolver:(RCTPromiseResolveBlock)resolve
                  rejecter:(RCTPromiseRejectBlock)reject)
{
  dispatch_async(dispatch_get_main_queue(), ^{
    [[EXPreviewZoomManager sharedInstance] setAppName:appName];
    resolve(@(YES));
  });
}
