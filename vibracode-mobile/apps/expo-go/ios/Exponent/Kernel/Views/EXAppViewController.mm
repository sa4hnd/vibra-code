...
- (void)appStateDidBecomeActive
{
  ...
  dispatch_async(dispatch_get_main_queue(), ^{
    ...
    // Re-apply preview zoom background color if zoomed out (fixes background color reset on foreground)
    EXPreviewZoomManager *zoomManager = [EXPreviewZoomManager sharedInstance];
    if ([zoomManager isZoomed] && _contentView && _contentView.superview) {
      [zoomManager reapplyBackgroundColor];
    }
  });
}
...
{
  // If splash screen has integrated progress, don't show separate window
  if (_managedAppSplashScreenViewProvider) {
    if (appLoader.shouldShowRemoteUpdateStatus) {
      // Status text updates (like "Checking for updates") go to splash screen
      NSString *statusText = nil;
      if (appLoader.remoteUpdateStatus == kEXAppLoaderRemoteUpdateStatusChecking) {
        statusText = @"Checking for updates";
      } else if (appLoader.remoteUpdateStatus == kEXAppLoaderRemoteUpdateStatusDownloading) {
        statusText = @"Downloading update";
      }
      if (statusText) {
        [_managedAppSplashScreenViewProvider updateProgress:0 status:statusText];
      }
    }
    // Don't show the separate window controller
    [self.appLoadingProgressWindowController hide];
  } else {
    // Fallback to original behavior for non-managed apps
    if (appLoader.shouldShowRemoteUpdateStatus) {
...
- (void)appLoader:(EXAbstractLoader *)appLoader didLoadBundleWithProgress:(EXLoadingProgress *)progress
{
  if (self->_appRecord.appManager.status != kEXReactAppManagerStatusRunning) {
    // Route progress to splash screen if it's visible, otherwise use window controller
    if (_managedAppSplashScreenViewProvider && [_managedAppSplashScreenViewProvider isShowingProgress]) {
      float progressPercent = ([progress.done floatValue] / [progress.total floatValue]);
      [_managedAppSplashScreenViewProvider updateProgress:progressPercent status:progress.status];
    } else if (_managedAppSplashScreenViewProvider) {
      // Splash screen exists but not yet showing progress - start showing it
      float progressPercent = ([progress.done floatValue] / [progress.total floatValue]);
      [_managedAppSplashScreenViewProvider updateProgress:progressPercent status:progress.status];
      // Don't show the separate window controller when splash has integrated progress
    } else {
      // Fallback to window controller if no splash screen provider
...
- (void)reactAppManagerIsReadyForLoad:(EXReactAppManager *)appManager
{
  ...
  // SIMPLE APPROACH: Always add contentView to self.view
  // The zoomOut method will move it to previewContainerView when needed
  ...
  // Notify preview zoom manager if contentView was replaced while zoomed out
  // or if we need to zoom out after reload (handles rapid reload case)
  // Use a delay to ensure view hierarchy is fully set up
  EXPreviewZoomManager *zoomManager = [EXPreviewZoomManager sharedInstance];
  if ([zoomManager isZoomed] || [zoomManager needsZoomAfterReload]) {
    NSLog(@"🔴 [AppVC] reactAppManagerIsReadyForLoad - isZoomed: %@, needsZoomAfterReload: %@ - calling handleContentViewReplacement",
          [zoomManager isZoomed] ? @"YES" : @"NO",
          [zoomManager needsZoomAfterReload] ? @"YES" : @"NO");
    dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(0.1 * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
      [zoomManager handleContentViewReplacement:reactView];
    });
  }
}
...
- (void)reactAppManagerFinishedLoadingJavaScript:(EXReactAppManager *)appManager
{
  ...
  // Check if we need to zoom out after a reload
  EXPreviewZoomManager *zoomManager = [EXPreviewZoomManager sharedInstance];
  BOOL needsZoom = [zoomManager needsZoomAfterReload];

  NSLog(@"🔴 [AppVC] JS finished loading - needsZoomAfterReload: %@, isZoomed: %@",
        needsZoom ? @"YES" : @"NO",
        [zoomManager isZoomed] ? @"YES" : @"NO");

  if (needsZoom) {
    [zoomManager setNeedsZoomAfterReload:NO];
    // Add a longer delay to ensure the view hierarchy is fully laid out and safe area is calculated
    dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(0.8 * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
      [[EXPreviewZoomManager sharedInstance] zoomOut];
    });
    return;
  }

  // Automatically zoom out when app finishes loading (only for non-home apps)
  if (!self.isHomeApp) {
    // Add a longer delay to ensure the view hierarchy is fully laid out and safe area is calculated
    dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(0.8 * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
      [[EXPreviewZoomManager sharedInstance] zoomOut];
...
{
  NSLog(@"🔴 [ErrorView] Retry tapped - setting needsZoomAfterReload and refreshing");
  // Set flag to zoom out after reload completes (if we were zoomed before error)
  [[EXPreviewZoomManager sharedInstance] setNeedsZoomAfterReload:YES];
  [self refresh];
}
...
- (void)_showErrorWithType:(EXFatalErrorType)type error:(nullable NSError *)error
{
  ...
  // Hide/remove loading cancel view when showing error
  // This prevents the "Opening project..." overlay from appearing on top of error screens
  if (_appLoadingCancelView) {
    [_appLoadingCancelView removeFromSuperview];
    _appLoadingCancelView = nil;
  }

  // Also hide the loading progress window
  [_appLoadingProgressWindowController hide];

  EXPreviewZoomManager *zoomManager = [EXPreviewZoomManager sharedInstance];

  NSLog(@"🔴 [ErrorView] Showing error - type: %ld", (long)type);

  if (_errorView && _contentView == _errorView) {
    // already showing, just update
    NSLog(@"🔴 [ErrorView] Updating existing error view");
    _errorView.type = type;
    _errorView.error = error;
  } else {
    NSLog(@"🔴 [ErrorView] Creating/showing error view, removing old contentView from superview");
    [_contentView removeFromSuperview];
    ...
    if (!_errorView) {
      ...
      NSLog(@"🔴 [ErrorView] Created new error view");
    }
    _errorView.type = type;
    _errorView.error = error;
    _contentView = _errorView;

    // SIMPLE APPROACH: Always add error view to self.view first
    // This ensures it has a valid frame and is in the view hierarchy.
    // Let the zoom manager handle moving it to the correct container if needed.
    NSLog(@"🔴 [ErrorView] Adding error view to self.view (simple approach)");
    _errorView.frame = self.view.bounds;
    [self.view addSubview:_contentView];

    // Let zoom manager handle proper positioning and splash screen hiding
    // This handles all zoom states correctly and prevents white screen issues
    [zoomManager handleErrorViewShown:_contentView];

    // If needsZoomAfterReload is set, trigger zoom out after a delay
    // This handles the case where refresh was pressed while zoomed, but the reload also failed
    if ([zoomManager needsZoomAfterReload]) {
      NSLog(@"🔴 [ErrorView] needsZoomAfterReload is YES, triggering zoomOut for error view");
      [zoomManager setNeedsZoomAfterReload:NO];
