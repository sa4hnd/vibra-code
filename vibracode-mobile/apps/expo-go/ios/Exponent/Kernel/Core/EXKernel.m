...
- (void)viewController:(__unused EXViewController *)vc didNavigateAppToVisible:(EXKernelAppRecord *)appRecord
{
  ...
  if (appRecord != appRecordPreviouslyVisible) {
    ...
    if (_visibleApp && _visibleApp != _appRegistry.homeAppRecord) {
      ...
      // Show orange menu when a non-home app becomes visible
      dispatch_async(dispatch_get_main_queue(), ^{
        [[EXOrangeMenuManager sharedInstance] showOrangeMenu];
      });
    } else if (_visibleApp == _appRegistry.homeAppRecord) {
      // Hide orange menu when home app becomes visible
      dispatch_async(dispatch_get_main_queue(), ^{
        [[EXOrangeMenuManager sharedInstance] hideOrangeMenu];
      });
    }
  }
}
