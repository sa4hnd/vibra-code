...
jest.mock('../../doctor/ngrok/NgrokResolver', () => {
  const instance: NgrokInstance = {
    ...
    connect: jest.fn(async () => 'https://www.vibracodeapp.com'),
    kill: jest.fn(),
  };
...
describe('getActiveUrl', () => {
  it(`is loaded on start`, async () => {
    ...
    expect(ngrok.getActiveUrl()).toEqual('https://www.vibracodeapp.com');
  });
});
...
describe('startAsync', () => {
  ...
  it(`starts`, async () => {
    const { ngrok } = createNgrokInstance();
    expect(await ngrok._connectToNgrokAsync()).toEqual('https://www.vibracodeapp.com');
  });
  ...
  it(`fixes invalid URL error by changing the randomness`, async () => {
    ...
    const connect = jest
      ...
      .mockImplementationOnce(() => 'https://www.vibracodeapp.com');
    ngrok.resolver.resolveAsync = jest.fn(async () => ({ connect }) as any);
