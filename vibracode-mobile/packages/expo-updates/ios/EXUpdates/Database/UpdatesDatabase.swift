...
public final class UpdatesDatabase: NSObject {
  ...
  public func addUpdate(_ update: Update, config: UpdatesConfig) throws {
    // CRITICAL FIX: Use INSERT OR REPLACE to handle duplicate update IDs gracefully
    // This prevents SQLite UNIQUE constraint errors when the same project is opened multiple times
    // The update ID comes from the manifest server, so opening the same project URL
    // will return the same manifest ID, causing duplicates without this fix
    let sql = """
