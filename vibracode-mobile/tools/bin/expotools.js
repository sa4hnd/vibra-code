...
function getSpawnOutputLines(result) {
  if (!result || !Array.isArray(result.output)) {
    ...
    if (result?.stderr) {
      fallbackOutput.push(result.stderr);
    }
    if (result?.stdout) {
      fallbackOutput.push(result.stdout);
    }
    if (fallbackOutput.length === 0 && result?.message) {
      fallbackOutput.push(result.message);
    }

    return fallbackOutput.length > 0 ? fallbackOutput : ['<no output>'];
  }

  return result.output.flatMap((line) => line.split('\n'));
}
